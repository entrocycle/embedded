...
完成本章后，你应能从 BootROM/SPL/U-Boot 追到 Kernel、DTB、RootFS 和用户服务，证明系统实际加载的镜像与参数，并将一个故障定位到启动、设备描述、驱动绑定、权限或用户态集成层。

本章建立全链路模型，不替代 [主机到目标机开发闭环](linux-development-workflow.md)、驱动子系统文档和 BSP 构建手册。启动组件名称与顺序因 SoC、安全启动和发行方案而异。

完成证据：完整启动日志标注、运行时 DT 与源码/DTB 对照，以及一个从 DTS 到用户态访问的垂直链路。源码目录中的文件不能证明目标设备实际使用了该版本。
...
嵌入式 Linux 是多层系统，任何一次用户态失败都可能来自启动参数、设备树、内核配置、驱动 probe、权限、udev、systemd、库依赖或应用自身。学习 Linux 全链路时，不要只追“怎么配置”，要建立跨层证据路径。

一个典型问题可以按下面路径收敛：

```text
硬件是否存在 -> 设备树是否描述 -> 驱动是否匹配 -> probe 是否成功
-> 设备节点/子系统接口是否出现 -> 权限是否允许 -> 服务是否正确使用
```

每一层都要有证据：启动日志、`/proc/device-tree`、`dmesg`、`lsmod`、`/sys`、`/dev`、`strace`、systemd journal 和应用日志。不要在缺少下层证据时直接修改上层业务代码。

开放问题：

1. 为什么用户态读 `/dev/i2c-1` 失败，不能直接判断 I2C 硬件有问题？
2. 如果同一份 RootFS 在两块板上表现不同，应如何区分 DTB、内核配置、硬件版本和运行时配置差异？
3. 为什么把业务逻辑塞进内核驱动会让 OTA、调试、崩溃隔离和安全审计更困难？
