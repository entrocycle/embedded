...
完成本章后，你应能用调度时间线和响应时间分析解释任务能否满足 deadline，根据数据/同步语义选择原语，并为队列满、优先级反转、栈不足和 ISR 交接设计可观察实验。

本章强调可迁移机制，但 API、调度选项、中断规则和内存管理取决于具体 RTOS 与版本。响应时间公式依赖周期/固定优先级等假设；存在自挂起、复杂资源协议、非周期干扰或多核时必须扩展模型。

完成证据：任务参数表、至少一条手工调度推演、一次 trace 对照、队列满和优先级反转实验。仓库 `labs/ring-buffer` 可验证 FIFO 边界，但不证明 RTOS 队列调度与并发实现。
...
“优先级看起来合理”不能证明任务一定按时完成。对周期任务或最小到达间隔已知的事件任务，先为每个任务记录：

| 参数 | 含义 | 证据来源 |
|---|---|---|
| `T` | 周期或最小到达间隔 | 需求、协议或实测事件间隔 |
| `D` | 相对截止期 | 控制、安全或业务需求 |
| `C` | 最坏执行时间 WCET | 最大输入、错误路径和目标优化级别下的测量与静态分析 |
| `B` | 被低优先级临界区阻塞的最大时间 | 锁持有时间、关中断区间和总线占用上界 |
| `J` | 释放抖动 | 中断、定时器和上游处理带来的最大发布延迟 |

对固定优先级抢占调度，可以迭代估算任务 `i` 从实际释放到完成的最坏响应时间：

```text
R_i(0)   = C_i + B_i
R_i(n+1) = C_i + B_i
           + sum(ceil((R_i(n) + J_j) / T_j) * C_j), j in hp(i))
```

`hp(i)` 是所有比任务 `i` 优先级高的任务。迭代值不再变化且 `R_i <= D_i`，才说明这组假设下满足截止期；一旦超过 `D_i` 就可以停止并判定失败。若截止期是相对理想采样时刻而不是实际任务释放时刻，还要把任务自身的释放抖动计入端到端延迟。

数值例子，优先级从上到下排列：

| 任务 | `T` | `D` | `C` | `B` | `J` |
|---|---:|---:|---:|---:|---:|
| protection | 5 ms | 5 ms | 1 ms | 0 | 0 |
| sample | 10 ms | 10 ms | 2 ms | 0 | 1 ms |
| control | 20 ms | 20 ms | 3 ms | 2 ms | 0 |

分析 `control`：

```text
R(0) = 3 + 2 = 5 ms
R(1) = 5 + ceil(5 / 5) * 1 + ceil((5 + 1) / 10) * 2 = 8 ms
R(2) = 5 + ceil(8 / 5) * 1 + ceil((8 + 1) / 10) * 2 = 9 ms
R(3) = 5 + ceil(9 / 5) * 1 + ceil((9 + 1) / 10) * 2 = 9 ms
```

因此在这些上界可信的前提下，`control` 的最坏响应时间是 9 ms，小于 20 ms 截止期。这里的 2 ms 阻塞即使使用优先级继承也不会消失；继承只限制优先级反转的放大，不会消除已经发生的临界区阻塞。

这项分析仍依赖输入质量：平均执行时间不能代替 `C`，正常路径不能代替错误路径，ISR 执行时间不能漏算。未建模成任务的 ISR 可作为更高优先级干扰单独加入；DMA、总线争用、cache miss 和长临界区也要进入 `C` 或 `B`。最终还要用 trace 测量响应时间分布，并通过最大负载和故障注入验证分析假设。
...
{
    sensor_msg_t msg;
    static uint32_t sensor_read_errors;
    static uint32_t sensor_queue_full;

    for (;;) {
        msg.timestamp = xTaskGetTickCount();
        if (sensor_read(&msg.temperature, &msg.humidity) != SENSOR_OK) {
            sensor_read_errors++;
        } else if (xQueueSend(sensor_queue, &msg, pdMS_TO_TICKS(10)) != pdPASS) {
            sensor_queue_full++;
        }
...
示例中的两个计数器只有 `sensor_task` 写；若诊断任务需要读取，应通过受保护的快照接口导出，不能因为是统计值就忽略数据竞争。实际项目还应定义传感器失败时是保留上一值、发送无效标记还是跳过本次上报。
...
RTOS 的价值不是“可以同时做很多事”，而是把系统中的时间、资源和等待关系显式化。任务、队列、信号量、事件组和定时器都只是工具；真正重要的是它们是否让系统的截止期、背压、错误恢复和诊断边界更清晰。

设计 RTOS 系统时，先画阻塞链而不是先写任务代码：

```text
任务 -> 等待对象 -> 拥有者 -> 最坏持有时间 -> 超时策略 -> 失败后动作
```

这张链路能暴露三类问题：

- 无界等待：某任务等待锁、队列或外设，但没有超时和恢复路径。
- 隐式优先级反转：高优先级任务被低优先级任务间接阻塞，中间还有其他任务抢占。
- 诊断断点缺失：系统卡住时，无法知道每个任务在等什么、等了多久、资源由谁持有。

RTOS 学习的关键是从 API 记忆升级为调度推理。每次创建任务都要能说明它的周期、截止期、栈预算、阻塞点、共享资源和健康指标；否则任务数量增加只会让系统更难证明。
