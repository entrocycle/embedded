...
完成本章后，你应能从需求与风险划分模块和依赖，定义数据、控制和错误流，为接口写所有权、并发、超时与版本契约，并通过变化场景检验边界是否真正隔离硬件、协议和业务变化。

本章给出设计工具而非通用目录答案。过度抽象会增加间接调用、状态和诊断成本；抽象不足会让变化扩散。架构质量必须由具体变化、故障和资源预算检验。

完成证据：五张项目图中的模块/数据/错误图、一个完整接口契约、两项架构取舍记录，以及“更换传感器/RTOS/云协议”影响分析。
...
项目架构的质量，往往在异常路径和需求变化时才显现。功能开发顺利时，紧耦合也能跑；一旦更换传感器、增加 OTA、切换云平台或定位现场问题，边界不清的代码会迅速变成维护负担。

设计模块边界时，建议按三条流画图：

- 数据流：采样、转换、缓存、上报、显示和存储如何流动。
- 控制流：命令、配置、状态切换和执行器动作如何触发。
- 错误流：底层错误如何上报、聚合、降级、记录和暴露给用户或运维。

很多架构图只画数据流，导致错误和控制路径在代码里随意蔓延。真正可维护的架构必须让错误路径和控制路径同样显性化。

开放问题：

1. 如果明天更换传感器型号，哪些模块应该不变？如果变化扩散很大，说明边界哪里有问题？
2. 错误码应该面向底层机制、上层业务还是现场诊断？如何避免所有错误都变成 `UNKNOWN`？
3. 为了方便调试暴露底层接口，会不会破坏抽象边界？如何设计受控的 debug 通道？
